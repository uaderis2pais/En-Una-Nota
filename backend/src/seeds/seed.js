...
const runSeed = async () => {
  try {
    console.log('🌱 Iniciando la siembra (Seed) de datos en PostgreSQL...');
    await initDatabase();

    const createdSongIds = [];

    // 1. Insertar canciones en la tabla 'canciones'
    for (const song of SEED_SONGS) {
      const res = await pool.query(
        `INSERT INTO canciones (titulo, artista, album, anio, portada_url, audio_url, start_time)
         VALUES ($1, $2, $3, $4, $5, $6, $7)
         RETURNING id, titulo;`,
        [song.title, song.artist, song.album, song.year, song.coverUrl, song.audioUrl, song.startTime]
      );
      createdSongIds.push(res.rows[0].id);
      console.log(`  🎵 Canción registrada: "${res.rows[0].titulo}" (ID: ${res.rows[0].id})`);
    }

    // 2. Programar canciones diarias para hoy y los próximos días
    const today = new Date();
    for (let i = 0; i < createdSongIds.length; i++) {
      const dateObj = new Date(today);
      dateObj.setDate(today.getDate() + i);
      const fechaStr = dateObj.toISOString().split('T')[0];
      const cancionId = createdSongIds[i];

      await pool.query(
        `INSERT INTO cancion_diaria (fecha, cancion_id)
         VALUES ($1, $2)
         ON CONFLICT (fecha) DO UPDATE SET cancion_id = EXCLUDED.cancion_id;`,
        [fechaStr, cancionId]
      );

      console.log(`  📅 Canción asignada para la fecha ${fechaStr} -> Song ID ${cancionId}`);
    }
